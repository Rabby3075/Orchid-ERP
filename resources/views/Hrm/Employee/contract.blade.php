    ...
    <div class="row flex-nowrap">
        <div class="col-auto col-md-3 col-xl-2 px-sm-2 px-0 bg-light">
            ...
            <ul class="nav nav-pills flex-column mb-sm-auto mb-0 align-items-center align-items-sm-start" id="menu">
                    ...
                    <li>
                        <a href="/Employee-contract/{{$user->id}}" class="nav-link px-0 align-middle active p-2">
        ...
        <div class="col py-3">
            <!-- Content Area -->
            <div class="card bg-white">
            <h4 class="card-header" id="header">Add New Contract</h4>
                <div class="card-body bg-white">
                <form action="{{route('ContractPost')}}" id="StoreInfo" class="form-group" method="POST" enctype="multipart/form-data">
                <input class="form-control" type="hidden" name="id" id="id" value="{{$user->id}}">
                <input class="form-control" type="hidden" name="contractId" id="contractId">
                                @csrf
                                <div class="form-group row">
                                    ...
                                    <div class="col-md-6">
                                    <label for="ctitle">Contract Title</label>
                                        <input class="form-control" type="text" name="ctitle" id="ctitle" placeholder="Contract Title">
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <div class="col-md-6">
            ...
            <div class="card bg-white">
            <h4 class="card-header">list All Contract</h4>
            <div class="card-body bg-white">
                            ...
                            <table class="table table-bordered mb-0" id="myTable">
                                ...
                                @foreach($contracts as $contract)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="javascript:void(0)" class="btn btn-success btn-sm me-1" title="Update" id="update" data-url="{{route('GetContract',$contract->id)}}">
                                                <i class="fa fa-edit"></i>
                                            </a>
                                            <a  class="btn btn-danger btn-sm" id="delete" title="Delete" href="javascript:void(0)" data-url="{{route('GetContract',$contract->id)}}">
...
<!--Side Bar code-->
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title text-danger">Contract Delete</h5>
                    </div>
                    <div class="modal-body">
                        <span class="text-dark">Are you sure to delete this Contract?</span>
                    </div>
                    <div class="modal-footer">
                        <form action="{{route('DeleteContract')}}" class="form-group" method="post" enctype="multipart/form-data">
                            {{csrf_field()}}
        ...
        <script>
            ...
            $('body').on('click', '#update', function () {
                $('#header').text('Update Contract')
                $('#submit').val('Update')
                $('#StoreInfo').attr('action','/Employee-Contract-Update');
                var userURL = $(this).data('url');
                $.get(userURL, function (data) {
                    $('#contractId').val(data.id);
                    $('#ctype').val(data.ContractType).change();
                    $('#ctitle').val(data.ContractTitle);
                    $('#from').val(data.From);
                    $('#to').val(data.To);
        ...
        <script>
            function create()
            {
                ...
                Toast.fire({
                    icon: 'success',
                    title: 'Contract Inserted Successfully'
                })
            }
            function error()
        {
